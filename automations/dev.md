...
**Copy quality — sector jargon:** Before finalising any user-facing copy, apply the "say it aloud" test from `rules/CONTENT_RULES.md` Section G. Every POC touches a real-world domain — caregiving, legal aid, food rescue, health, housing. Those domains have professional vocabularies that users do not speak. Words like "handoff", "intake", "support route", "eligibility scheme", "escort", "dispatch", or "care shifts" are practitioner language, not resident language. Read every sentence as if you are explaining the app to a stranger on the street. If it sounds like a form or a case file, rewrite it. Also: all apostrophes in JSX must be written as `&apos;` — unescaped `'` characters cause build failures at the lint stage.
